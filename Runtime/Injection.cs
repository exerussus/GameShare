    ...
    public static class SharedObjectInjector
    {
        ...
        public static void InjectSharedObjects(object target, GameShare gameShare)
        {
            ...
            while (targetType != null && targetType != typeof(object))
            {
                ...
                // Свойства с сеттерами или с доступом к бэкинг-филду
                foreach (var property in properties)
                {
                    ProcessInjection(property, target, gameShare,
                        ...
                        sharedObject =>
                        {
                            ...
                            {
                                // если есть сеттер — просто устанавливаем
                                property.SetValue(target, sharedObject);
                            }
                            else
                            {
                                // иначе ищем авто-бэкинг-филд
                                var backingFieldName = $"<{property.Name}>k__BackingField";
                                var backingField = targetType.GetField(backingFieldName, BindingFlags.NonPublic | BindingFlags.Instance);
